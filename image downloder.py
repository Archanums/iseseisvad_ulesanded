...
def download_files(url): # Defineerib funktsiooni
    ...
    with requests.get(url, stream=True) as r: # Defineerib r, mis on requests.get funktsiooni parameeter
        ...
        with open("C:/Users/marcello.sirikov/Desktop/"+local_filename, 'wb') as f: # Defineerib f, mis on local_filename
    ...
    print("File saved as "+ local_filename) # Printib "File salvestuse as "+ local_filename

while 1: # Korduv while loop
    print("Welcome to the image downloader") # Printib "Welcome to the image downloader"
    image_url = input(str("Image url : ")) # Küsib kasutaja sisestamist
    download_files(image_url) # Funktsiooni käivitamine
    print("") # Printib ""